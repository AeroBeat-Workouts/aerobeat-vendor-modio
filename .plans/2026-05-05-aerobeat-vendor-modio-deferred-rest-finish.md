...
- `.testbed/tests/fixtures/mod_events.json`
- `.testbed/tests/fixtures/mods_events.json`
- `README.md`
- `.plans/2026-05-05-aerobeat-vendor-modio-deferred-rest-finish.md`

**Status:** ✅ Complete

**Results:** Slice A coder completed on bead `oc-3ee` for the mod authoring CRUD endpoints `POST /games/{game-id}/mods`, `POST /games/{game-id}/mods/{mod-id}`, and `DELETE /games/{game-id}/mods/{mod-id}`. The implementation stayed a thin REST wrapper: bearer-authenticated multipart builders were added for create/update, bodyless delete normalization was added for `204 No Content`, and the wrapper validates only documented field names plus the REST-documented enum/value constraints used by those pages. Tests were extended at both request-builder and transport-execution layers, README coverage notes were updated, and repo validation ran via `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` with all 68 tests passing (1 pre-existing float/int warning in checkout/S2S coverage). Implementation commit: `5a2ffbe` (`Add REST wrappers for mod authoring CRUD`). Slice A QA on bead `oc-qff` independently checked the local official REST corpus pages `add-mod.api.mdx`, `edit-mod.api.mdx`, and `delete-mod.api.mdx` against the wrapper and found one concrete transport mismatch: the docs require repeated `metadata[]` multipart parts for mod-profile metadata during add/edit, but the new wrapper emitted `metadata_kvp[]`. QA applied the minimum fix in `src/modio_vendor_adapter.gd` to remap the validated `metadata_kvp` input onto the docs-correct multipart field name `metadata[]`, updated the request-builder and transport assertions in `.testbed/tests/test_modio_vendor_adapter.gd` and `.testbed/tests/test_modio_http_transport.gd`, refreshed the mod-authoring seam note in `README.md`, and reran `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` with all 68 tests passing again (same single pre-existing float/int warning in checkout/S2S coverage). Files touched by QA: `src/modio_vendor_adapter.gd`, `.testbed/tests/test_modio_vendor_adapter.gd`, `.testbed/tests/test_modio_http_transport.gd`, `README.md`, and this plan file. QA commit: `d8d50c1` (`QA fix mod authoring metadata multipart key`). Slice A audit on bead `oc-7j8` independently truth-checked the same three endpoints against `REF-04`, `REF-06`, `REF-07`, and this plan. Audit result: the QA metadata remap is correct; add/edit now accept the documented client-facing `metadata_kvp` input while encoding the actual multipart parts as repeated `metadata[]`, auth stays bearer-only on create/update/delete, create/update stay `multipart/form-data`, delete stays bodyless with `204` normalization, response normalizers return the expected created/updated/deleted flags plus normalized `Mod Object` payloads, and no drift-blocked `/me/iap/*/sync` or other non-slice behavior leaked into this surface. The current request-builder coverage and transport assertions are sufficient for this slice: `.testbed/tests/test_modio_vendor_adapter.gd` checks field allowlists, required create fields, `metadata[]` remapping, and normalized write responses, while `.testbed/tests/test_modio_http_transport.gd` checks final encoded multipart/form-data bodies, bearer headers, and bodyless delete execution. The auditor reran `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit`; all 68 tests passed again with the same single pre-existing float/int warning in checkout/S2S coverage. Minimum-fix outcome: no additional code changes were required during audit. Files touched by audit: this plan file only. Slice A audit is complete. Slice B coder then completed on bead `oc-pu0` for `POST /games/{game-id}/media` and `DELETE /games/{game-id}/collections/{collection-id}/mods` using the local official REST pages `add-game-media.api.mdx` and `delete-collection-mods.api.mdx` as source of truth. The implementation stayed literal to the REST corpus: `build_add_game_media_request(...)` now emits bearer-authenticated `multipart/form-data` with only the documented `logo`, `icon`, `header`, and repeated `redirect_uris[]` fields, while `build_delete_collection_mods_request(...)` now emits bearer-authenticated `DELETE` + `application/x-www-form-urlencoded` with required repeated `mod_ids[]` positive integers. Response normalization was added for the docs-shaped game-media message payload on `200 OK` and the bodyless collection-mod delete `204 No Content` result. Tests were extended at both request-builder and transport-execution layers, a docs-shaped success fixture was added, and README coverage notes were updated. Files touched by the Slice B coder implementation: `src/modio_vendor_adapter.gd`, `.testbed/tests/test_modio_vendor_adapter.gd`, `.testbed/tests/test_modio_http_transport.gd`, `.testbed/tests/fixtures/add_game_media_success.json`, and `README.md`. Validation run: `godot --headless --path .testbed --script res://tests/validate_scaffold.gd` (passed) and `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` (71/71 passing, 2346 asserts, same one pre-existing unrelated float/int warning in checkout/S2S coverage). Slice B coder commit: `3b8ab11` (`Add game media and collection mod delete wrappers`). Slice B QA on bead `oc-6yp` independently checked `REF-04` pages `add-game-media.api.mdx` and `delete-collection-mods.api.mdx` against the current wrapper and found one concrete multipart transport defect in the new game-media endpoint: the wrapper still accepted plain string values like `@/tmp/game-header.png`, which the transport serialized as a text form field instead of the docs-required binary multipart file part. QA applied the minimum fix in `src/modio_vendor_adapter.gd` to require truthful multipart file-part objects (`filename`, optional `content_type`, raw byte `data`) specifically for `logo`, `icon`, and `header` on `POST /games/{game-id}/media`, while keeping `redirect_uris[]` and the collection-mod delete form behavior unchanged. QA updated `.testbed/tests/test_modio_vendor_adapter.gd` and `.testbed/tests/test_modio_http_transport.gd` to assert binary file-part encoding for all three media fields plus rejection of string-path shorthands, refreshed the Slice B seam note in `README.md`, and reran `godot --headless --path .testbed --script res://tests/validate_scaffold.gd` plus `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit`; validation passed with 71/71 tests green, 2351 asserts, and the same one pre-existing unrelated float/int warning in checkout/S2S coverage. Files touched by Slice B QA: `src/modio_vendor_adapter.gd`, `.testbed/tests/test_modio_vendor_adapter.gd`, `.testbed/tests/test_modio_http_transport.gd`, `README.md`, and this plan file. QA commit: `f3c2bba` (`QA fix game media multipart file enforcement`). Slice B audit on bead `oc-kka` independently truth-checked `POST /games/{game-id}/media` and `DELETE /games/{game-id}/collections/{collection-id}/mods` against `REF-04`, `REF-06`, `REF-07`, and this plan. Audit result: the QA multipart fix is correct; game media now requires truthful multipart file-part objects for `logo`, `icon`, and `header`, keeps bearer-only auth, keeps `multipart/form-data` transport, preserves repeated `redirect_uris[]` encoding, and normalizes only the docs-shaped `Message Object` on `200 OK`. Collection-mod removal stays bearer-authenticated `DELETE` + `application/x-www-form-urlencoded`, requires documented repeated `mod_ids[]` positive integers, and normalizes the bodyless `204 No Content` response without adding collection-management behavior beyond the REST page. Independent transport/test review confirmed the request-builder coverage and execution assertions are sufficient for this slice: `.testbed/tests/test_modio_vendor_adapter.gd` checks allowlisted fields, binary file-part validation, redirect URI validation, delete-body integer validation, and normalized write responses, while `.testbed/tests/test_modio_http_transport.gd` checks final bearer headers, multipart binary part assembly for all three media fields, repeated `redirect_uris[]`/`mod_ids[]` encoding, and rejection of string-path media shorthands. The auditor reran `godot --headless --path .testbed --script res://tests/validate_scaffold.gd` and `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit`; validation passed with 71/71 tests green, 2351 asserts, and the same one pre-existing unrelated float/int warning in checkout/S2S coverage. Minimum-fix outcome: no additional code changes were required during audit. Files touched by audit: this plan file only. Slice B audit is complete. Slice C coder then completed on bead `oc-7il` for the two non-deprecated mod event feeds `GET /games/{game-id}/mods/{mod-id}/events` and `GET /games/{game-id}/mods/events`, using the local official REST pages `get-mod-events.api.mdx` and `get-mods-events.api.mdx` as source of truth while explicitly leaving deprecated `GET /me/events` deferred per Derrick. The implementation stayed thin and docs-literal: `build_mod_events_request(...)` now emits the mod-scoped public `GET` wrapper with only shared pagination inputs, `build_mods_events_request(...)` now emits the documented global mod-events query subset (`id`, `mod_id`, `user_id`, `date_added`, `event_type`, `latest`, `subscribed`) without leaking unrelated listing filters, the query model was extended only far enough to represent those event-feed fields, and bearer auth is promoted only when the docs-specific `subscribed=true` filter requires authenticated-user context. Response normalization was added for both event-feed list payloads using the documented `Mod Event Object` fields (`id`, `mod_id`, `user_id`, `date_added`, `event_type`) without inventing higher-level semantics. Tests were extended at request-builder, transport-execution, and normalization layers; new event fixtures were added for both endpoints; and README coverage notes were updated to document the authenticated `subscribed=true` caveat plus the decision to keep mod-scoped events pagination-only per the current local docs page. Files touched by the Slice C coder implementation: `src/modio_vendor_adapter.gd`, `src/models/modio_listing_query.gd`, `.testbed/tests/test_modio_vendor_adapter.gd`, `.testbed/tests/test_modio_http_transport.gd`, `.testbed/tests/fixtures/mod_events.json`, `.testbed/tests/fixtures/mods_events.json`, and `README.md`. Validation run: `godot --headless --path .testbed --import` (passed), `godot --headless --path .testbed --script res://tests/validate_scaffold.gd` (passed), and `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` (72/72 passing, 2396 asserts, same one pre-existing unrelated warning in checkout/S2S coverage). Slice C coder commit: `6a35dfe` (`Add mod event feed REST wrappers`). Slice C QA on bead `oc-mc5` independently rechecked `REF-04` pages `get-mod-events.api.mdx`, `get-mods-events.api.mdx`, and `get-user-events.api.mdx` against the shipped wrapper, README, and tests. Paths, documented filter allowlists, response normalization, and the intentional deferral of deprecated `GET /me/events` all held. QA found one concrete auth-seam defect in `GET /games/{game-id}/mods/events`: when callers requested the docs-specific `subscribed=true` filter without an access token, `build_mods_events_request(...)` still fell back to `_build_public_query()`, leaking `api_key` into a request that the docs define as authenticated-user-only for that filter. QA applied the minimum fix in `src/modio_vendor_adapter.gd` so `subscribed=true` forces bearer-mode request shaping with no `api_key` fallback and now surfaces a local validation error (`subscribed=true requires an authenticated user access token`) when the caller omits a token. QA updated `.testbed/tests/test_modio_vendor_adapter.gd` and `.testbed/tests/test_modio_http_transport.gd` to cover both the happy-path bearer promotion and the no-token non-leak/validation case, refreshed the Slice C seam note in `README.md`, and preserved the mod-scoped events endpoint as pagination-only per the current local docs page. `GET /me/events` remains intentionally unimplemented and documented as deprecated-for-new-games comparison/deferred scope only, matching Derrick’s checkpoint decision and the local REST page guidance to use `GET /me/subscribed` instead. Files touched by Slice C QA: `src/modio_vendor_adapter.gd`, `.testbed/tests/test_modio_vendor_adapter.gd`, `.testbed/tests/test_modio_http_transport.gd`, `README.md`, and this plan file. Validation rerun: `godot --headless --path .testbed --script res://tests/validate_scaffold.gd` (passed) and `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` (73/73 passing, same single pre-existing unrelated float/int warning in checkout/S2S coverage). QA commit: `fb0a321` (`QA fix mods events auth fallback`). Slice C audit on bead `oc-s47` independently truth-checked `GET /games/{game-id}/mods/{mod-id}/events` and `GET /games/{game-id}/mods/events` against `REF-04`, `REF-06`, the repo fixture corpus under `.testbed/tests/fixtures/` (note: the repo does not have a `tests/fixtures/docs` path; the active fixture corpus remains `.testbed/tests/fixtures/` as already recorded in `REF-07`), and this plan. Audit result: the QA auth fix is correct; `build_mod_events_request(...)` stays on the exact documented path `/games/{game-id}/mods/{mod-id}/events`, keeps public read shaping, and allows only pagination (`_limit`, `_offset`) beyond the required path ids, which matches the current local docs page. `build_mods_events_request(...)` stays on the exact documented path `/games/{game-id}/mods/events`, allows only the documented query subset (`id`, `mod_id`, `user_id`, `date_added`, `event_type`, `latest`, `subscribed`) plus shared pagination, and no unrelated listing/search filters leak into the request. Pagination behavior is truthful for both endpoints: request builders preserve `_limit` / `_offset`, transport tests lock the final encoded URLs, and response normalization preserves `result_count`, `result_offset`, `result_limit`, `result_total`, and derived page state through the shared list normalizer. Auth handling is truthful: the mod-scoped feed remains a public query-auth read, the global mods feed remains public unless `subscribed=true` is requested, and `subscribed=true` now correctly promotes to bearer-only request shaping with no `api_key` fallback; when no access token is present the wrapper surfaces the local validation error `subscribed=true requires an authenticated user access token` and the transport tests confirm no auth leakage occurs. Response normalization is minimal and docs-shaped for both feeds, returning only normalized `Mod Event Object` fields (`id`, `mod_id`, `user_id`, `date_added`, `event_type`) plus the existing shared list envelope/page metadata. README truth check passed for the Slice C seam note. Deprecated `GET /me/events` remains intentionally deferred and unimplemented, which matches Derrick’s locked checkpoint and the local REST doc deprecation guidance pointing new games at `GET /me/subscribed` instead. No deprecated or drift-blocked behavior leaked into the Slice C surface, including no `/me/iap/*/sync` or user-events implementation creep. Independent coverage review confirmed the current test/fixture set is sufficient for this slice: `.testbed/tests/test_modio_vendor_adapter.gd` checks path/auth/query shaping, supported-filter allowlists, and unauthenticated `subscribed=true` validation; `.testbed/tests/test_modio_http_transport.gd` checks final encoded public and bearer URLs plus the auth no-leak case; `.testbed/tests/fixtures/mod_events.json` and `.testbed/tests/fixtures/mods_events.json` match the current docs-shaped `Mod Event Object` envelope used by the normalizers. Validation rerun during audit: `godot --headless --path .testbed --script res://tests/validate_scaffold.gd` (passed) and `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` (73/73 passing, 2413 asserts, same single pre-existing unrelated float/int warning in checkout/S2S coverage). Minimum-fix outcome: no additional code changes were required during audit. Files touched by audit: this plan file only. Audit commit: pending after plan update. Slice C audit is complete.
...
**Results:** Wrote and then retargeted `docs/modio-unity-vs-vendor-wrapper-gap-2026-05-05.md` into a practical AeroBeat planning note rather than a generic SDK audit. The document still records the final wrapper truth accurately — `aerobeat-vendor-modio` wraps **133 of 134** documented REST endpoint pages, with only deprecated `GET /me/events` intentionally unwrapped — but it now answers the planning questions AeroBeat actually cares about: what the wrapper already covers, what Unity still provides above the REST seam, what should become Godot responsibilities, which Unity UI/examples/helpers should be treated as references instead of direct port targets, which drift areas still need mod.io clarification, and what real live integration testing will require beyond mocked/unit-style/docs-truth validation. The new live-testing section is explicit that current repo validation proves wrapper truth, not production integration, and that a real test pass likely needs private local env/config secrets plus a safe live-test scope before destructive or monetization-sensitive flows are exercised. User-facing pointers in the doc now prefer Nerve-friendly `/workspace/...` paths. Validation for this docs-only retargeting was lightweight and appropriate: `git diff --check` passed cleanly, and the updated markdown was spot-checked against the audited wrapper state plus the local Unity/doc references. Files touched in this slice: the updated comparison doc and this plan file. Commit/push for this retargeting work: `bf4405f` (`Refocus mod.io comparison doc for AeroBeat planning`), pushed to `origin/main`. 
...
**What We Built:** The remaining confirmed REST-backed wrapper slices are now landed, the final REST audit is complete, and the Unity SDK comparison write-up has been retargeted into an AeroBeat planning note. `aerobeat-vendor-modio` truthfully wraps **133 of 134** documented mod.io REST endpoint pages from `REF-04`. The only documented route still intentionally left out is deprecated `GET /me/events`, which remains explicitly deferred per Derrick’s decision and the deprecation language on the official page itself. The updated planning note at `docs/modio-unity-vs-vendor-wrapper-gap-2026-05-05.md` now explains the remaining gap in practical AeroBeat terms: the wrapper is basically done at the public REST seam, while the next real work is Godot runtime/integration behavior, unresolved monetization/auth drift that needs mod.io clarification, and private credential-backed live testing.

**Reference Check:** `REF-04` re-audited against `REF-06` and `REF-07` with no remaining non-deprecated confirmed REST-backed gaps found. Intentional documented deferral: `GET /me/events`. The new comparison document cross-checks that final REST result against `REF-05` and classifies the leftover Unity-vs-wrapper differences into: REST-covered functionality already wrapped, intentional documented deferral, and Unity-only / drift-blocked surfaces including `/me/iap/*/sync`, WSS/device-login auth, install orchestration, and monetization runtime abstractions.
...
- `cc4f908` - Record final REST audit results
- `b1c25a1` - Document Unity vs wrapper REST gap
- `0d16770` - Update plan with comparison results
- `bf4405f` - Refocus mod.io comparison doc for AeroBeat planning

**Lessons Learned:** The local REST corpus is stable enough to finish the vendor wrapper accurately when each slice stays docs-first and thin, but the remaining parity questions now live above or beside the REST seam. For Godot, the next risky areas are not ordinary endpoint wrapping; they are platform-store entitlement sync, WSS/device auth expectations, and how much Unity-style runtime/install behavior mod.io actually expects custom-engine clients to supply themselves.
