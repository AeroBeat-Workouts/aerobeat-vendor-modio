...
**Results:** Independent audit passed without needing an audit-sized fix. The Test-vs-Live auth session bucket fix is real in the controller/store seam and the validation evidence supports the claim. In `.testbed/scripts/modio_workout_browser_testbed.gd`, auth exchange, routine session persistence, restore-time bucket loading, and clear-session all now route through the active environment selected by the server selector instead of relying on stale in-memory environment state. `_load_initial_state()` resolves the stored environment first, `_load_session_bucket_into_state(environment)` restores `access_token`, `access_token_expires_at`, `user_id`, `email`, `last_requested_email`, and `browser_tab` from that environment-specific section, and `_sync_environment_from_selector(...)` is now the shared gate that keeps selector state, runtime state, manager rebuilds, and persisted `modio.environment` selection aligned.

The core bucket-routing claims checked out against both code and focused regression coverage. `_on_exchange_code_pressed()` persists through `_persist_session_state()` only after `_sync_environment_from_selector(false, true)` updates the active bucket selection, so a Test auth exchange now writes into `modio.test` rather than leaking into `modio.live`. `_on_clear_session_pressed()` similarly starts with `_sync_environment_from_selector(true, true)`, so clearing Test auth removes saved Test bearer/session keys from `modio.test` while leaving the Live bucket untouched. The QA follow-up fix is also correct: `_persist_session_state()` now writes `access_token_expires_at` only when it is a real positive timestamp, preventing a cleared expiry from being rewritten later as the bogus string `"0"`.
...
**Results:** QA passed without needing a follow-up fix. The saved-auth startup hydration seam now behaves correctly against the exact regression Derrick reported. Focused browser coverage in `.testbed/tests/test_modio_workout_browser_testbed.gd` proves that a saved future-dated bearer rehydrates into an authenticated state on startup, hydrating the expected non-public views through the restore path instead of waiting for manual refresh: `Profile` loads via the startup `/me` + wallet + purchases pass, `Workout Browser` and `Subscribed Workouts` both populate through the same `_fetch_listing_for_context(...)` seam used by manual refreshes, and `Upload Workout` restores as a usable authenticated surface with the saved tab intent and authenticated intro/status copy already present.

That directly satisfies the two critical truth checks for this QA slice. First, restored auth now hydrates workout and subscribed listings through the same real fetch path used by manual refresh rather than a fake post-auth UI shortcut. Second, `Upload Workout` no longer sits in a half-restored state on startup: when the saved `browser_tab` is `upload`, startup restore brings that tab back as the active browser tab and the upload surface comes up in authenticated copy/state without requiring any manual refresh affordance. The focused startup test covering `test_ready_restores_saved_auth_and_hydrates_non_public_views` stayed green, along with the adjacent auth-gating and restore-path guards.

QA reran: `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gtest=res://tests/test_modio_workout_browser_testbed.gd -gexit` (15/15 passing), `godot --headless --path .testbed --script res://tests/validate_modio_testbed_scenes.gd` (passed), `godot --headless --path .testbed --script res://tests/qa_verify_scene_output_updates.gd` (passed, with the same pre-existing non-failing `ObjectDB`/resources-at-exit warning), and the full `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` suite (126/126 passing, with the same pre-existing non-failing Float/Int comparison warning in `test_modio_vendor_adapter.gd`). No new regression or QA-sized defect was found in this saved-auth startup hydration slice.
...
**Results:** Independent audit passed without needing an audit-sized fix. The saved-auth startup hydration seam now truthfully hydrates the expected non-public views on startup instead of only restoring bearer/session state and waiting for manual refresh. In `.testbed/scripts/modio_workout_browser_testbed.gd`, `_restore_saved_runtime_state()` now routes authenticated restore through `_hydrate_restored_authenticated_views()`, which first refreshes `/me` + wallet + purchases and then explicitly hydrates the authenticated listing views before reapplying the auth-gated browser state. That means the startup path no longer leaves `Upload Workout` in a half-restored state while `Workout Browser` / `Subscribed Workouts` wait behind separate manual fetches.

The key truth check about fetch-path parity is also satisfied in code. Manual listing refresh still goes through `_fetch_listing(context)`, and that function delegates the real work to `_fetch_listing_for_context(context)`. Startup restored-auth hydration now uses that same `_fetch_listing_for_context(...)` helper for both `ModioWorkoutBrowserState.TAB_WORKOUT` and `ModioWorkoutBrowserState.TAB_SUBSCRIBED`, so startup hydration reuses the exact same request-building, auth gating, adapter normalization, listing-state update, and UI refresh seam as a manual fetch instead of duplicating or faking post-auth population. `Upload Workout` itself has no separate network refresh path, but the startup restore now brings it up through the authenticated browser-state/UI seam with the saved tab intent and authenticated intro/status copy already active.

Fresh audit validation supports the claim. I reran `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gtest=res://tests/test_modio_workout_browser_testbed.gd -gexit` (15/15 passing), `godot --headless --path .testbed --script res://tests/validate_modio_testbed_scenes.gd` (passed), and `godot --headless --path .testbed --script res://tests/qa_verify_scene_output_updates.gd` (passed, with the same pre-existing non-failing `ObjectDB`/resources-at-exit warning from the scene QA harness). The focused browser regression `test_ready_restores_saved_auth_and_hydrates_non_public_views` directly proves the restored-auth startup path now hydrates public/workout/subscribed listings, restores `Upload Workout` as the active tab, and shows authenticated upload copy without requiring a manual refresh. I did not find evidence of a regression in the non-public startup hydration seam.

---
### Task 31: Investigate and fix mod.io logo multipart upload encoding

**Bead ID:** `oc-j7lr`  
**SubAgent:** `primary` (for `coder`)  
**Role:** `coder`  
**References:** `REF-02`, `REF-04`, `REF-05`, `REF-06`  
**Prompt:** In `aerobeat-vendor-modio`, claim bead `oc-j7lr` on start with `bd update oc-j7lr --status in_progress --json`. Investigate why mod.io still rejects the `logo` field as non-image even when the testbed passes local file existence/image readability/minimum-dimension validation and the operator supplies absolute image paths. Compare the multipart upload encoding for the `logo` field against the working ZIP `filedata` seam, inspect adapter and HTTP transport multipart handling, identify the real root cause, and fix it if it is local. Keep diagnostics truthful if any provider-side requirement remains. Add focused regression coverage, run relevant validation, update this plan with what actually changed, commit and push by default, and close the bead with a clear reason.
...
**Results:** Root cause confirmed and fixed locally in the transport seam. The upload helper and adapter were still handing multipart file fields to the transport using the repo's older `"@/absolute/path"` convention, but `src/network/modio_http_transport.gd` was not interpreting that convention as a file upload. For multipart requests it encoded those values as ordinary text fields, so `logo` reached mod.io as the literal string path instead of an image file part (`Content-Disposition ... filename=...` plus image bytes). The ZIP `filedata` seam was using the same broken convention; it only looked more plausible in local tests because the old assertions checked for the literal `@/path` text in the body instead of proving a real file part was emitted.

The fix keeps the existing adapter/helper contract but makes it truthful at the transport boundary: multipart values that begin with `@` are now resolved to local files, loaded as raw bytes, assigned a filename, and given an inferred content type (`image/png`, `image/jpeg`, `image/gif`, `image/webp`, `application/zip`, or octet-stream fallback). Missing `@file` paths now fail early with a clear transport validation error instead of silently sending bogus text to mod.io. I did not need to widen the upload helper API; its existing local preflight stays useful, and the real fix now applies consistently to both `logo` and ZIP `filedata` authoring seams across the repo.

Focused regression coverage was updated to prove the real behavior instead of the old fake one. `test_modio_http_transport.gd` now writes real temp PNG/ZIP fixtures, asserts multipart requests contain `filename=` plus the correct `Content-Type`, asserts the raw request bytes include the actual file payload, and covers the missing-`@file` error path. `test_modio_workout_upload_flow.gd` now verifies the staged create/upload flow emits a real image part for `logo` and a real ZIP part for `filedata` while preserving the existing helper contract. Validation passed with `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gtest=res://tests/test_modio_http_transport.gd -gtest=res://tests/test_modio_workout_upload_flow.gd -gexit`, `godot --headless --path .testbed --import`, `godot --headless --path .testbed --script res://tests/validate_modio_testbed_scenes.gd`, and the full `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` suite (all passing; the pre-existing float/int GUT warning in `test_modio_vendor_adapter.gd` remains outside this slice).

---

### Task 32: QA logo multipart upload encoding fix

**Bead ID:** `oc-hvki`  
**SubAgent:** `primary` (for `qa`)  
**Role:** `qa`  
**References:** `REF-02`, `REF-04`, `REF-05`, `REF-06`  
**Prompt:** In `aerobeat-vendor-modio`, claim bead `oc-hvki` on start with `bd update oc-hvki --status in_progress --json`. Verify the logo multipart upload seam fix. Confirm the root cause is addressed or surfaced correctly, confirm valid logo uploads are encoded properly for mod.io, rerun relevant validation/tests, update this plan with QA findings, commit/push by default only if QA-sized fixes are needed, and close the bead with a clear reason.
...
### Task 33: Audit logo multipart upload encoding fix
...
**Results:** QA passed without needing a fix. The binary multipart transport seam now behaves truthfully in the normal success path for both raw octet-stream uploads and multipart form uploads with file parts. In `src/network/modio_http_transport.gd`, prepared/final requests now explicitly mark binary payloads with `body_is_binary`, keep the real `body_bytes` for dispatch, and expose only safe preview text in `body_string` (`[binary body omitted: ...]` for raw bytes and `[binary file omitted: ...]` placeholders for multipart file parts) instead of coercing PNG/ZIP/octet-stream payload bytes into UTF-8 text.

Focused regression coverage proves both halves of that claim. `.testbed/tests/test_modio_http_transport.gd` verifies raw binary part uploads set `body_is_binary`, keep the real bytes, and report the safe raw-body preview; it also verifies multipart `@file` uploads for image/ZIP paths emit real file parts while the debug preview contains only truthful placeholders plus filename/content-type context. `.testbed/tests/test_modio_workout_upload_flow.gd` confirms the staged workout authoring seam still uses the real multipart file bytes for `logo` and `filedata` while exposing only safe placeholders in the recorded request preview. That satisfies the QA goals that diagnostics stay truthful while transport continues to send the real body bytes.

Validation stayed green and the captured logs did not reproduce the prior UTF-8/Unicode parsing noise in the successful upload path. QA reran: `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gtest=res://tests/test_modio_http_transport.gd -gtest=res://tests/test_modio_workout_upload_flow.gd -gexit`, `godot --headless --path .testbed --import`, `godot --headless --path .testbed --script res://tests/validate_modio_testbed_scenes.gd`, `godot --headless --path .testbed --script res://tests/qa_verify_scene_output_updates.gd`, and the full `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` suite. All passed (`127/127`). The only remaining warnings are the same pre-existing non-failing ones already tracked elsewhere in the plan: the scene-harness `ObjectDB`/resources-at-exit warning and the float/int comparison warning in `test_modio_vendor_adapter.gd`. No UTF-8 multipart warning regression remained in this QA slice.
...
**Results:** Independent audit passed without needing an audit-sized fix. The real local cause from the previous coder slice is present in the transport and the fix is truthful: `src/network/modio_http_transport.gd` no longer coerces binary request payloads into UTF-8 debug strings for either raw `PackedByteArray` request bodies or multipart form bodies that include file parts. `_normalize_raw_request_body()` now keeps the real `body_bytes` while exposing a descriptive preview (`[binary body omitted: ...]`), and `_encode_multipart_body()` now intentionally builds two representations: the real binary `body_bytes` sent to mod.io plus a safe `body_string` preview composed of text fields, multipart headers, and explicit binary placeholders such as `[binary file omitted: ...]`. Prepared/final requests also carry a truthful `body_is_binary` flag, which matches the implementation claim and gives tests/diagnostics an explicit signal that the preview is intentionally non-literal.

I independently verified the evidence at both the transport seam and the higher-level upload helper seam. In `.testbed/tests/test_modio_http_transport.gd`, the binary multipart coverage proves raw octet-stream uploads and multipart `@file` uploads both set `body_is_binary`, preserve the real request bytes, and expose only safe preview text. In `.testbed/tests/test_modio_workout_upload_flow.gd`, the staged workout authoring flow still carries the actual PNG/ZIP bytes in `body_bytes` for `logo` and `filedata`, while the recorded request preview contains only truthful placeholders plus filename/content-type context. That satisfies the audit goals that diagnostics stay safe and honest while the transport continues to dispatch the real body bytes.

I reran the focused and broader validation myself and did not reproduce the prior UTF-8/Unicode parsing noise in the normal success path. Passing checks: `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gtest=res://tests/test_modio_http_transport.gd -gtest=res://tests/test_modio_workout_upload_flow.gd -gexit`, `godot --headless --path .testbed --import`, `godot --headless --path .testbed --script res://tests/validate_modio_testbed_scenes.gd`, `godot --headless --path .testbed --script res://tests/qa_verify_scene_output_updates.gd`, and the full `godot --headless --path .testbed --script addons/gut/gut_cmdln.gd -gdir=res://tests -ginclude_subdirs -gexit` suite (`127/127` passing). The only remaining warnings are the same pre-existing non-failing ones already tracked elsewhere in the plan: the float/int comparison warning in `test_modio_vendor_adapter.gd` plus the scene-harness `ObjectDB` / resources-at-exit warning. No UTF-8 multipart regression remained in this audit slice.
...
**What We Built:** The default mod.io browser testbed still uses the reusable helper-driven `Upload Workout` tab and now also seeds truthful metadata for vendor-seam testing instead of guessed taxonomy examples. The new cross-repo seam is in place: `aerobeat-tool-device-detection` now exposes a dedicated helper that normalizes detected-device JSON into stable mod.io metadata KVP pairs, and `aerobeat-vendor-modio` consumes that helper in the upload testbed to prefill deterministic device-derived metadata alongside explicit seeded fields like `aerobeat_version=1.0.0`. Public AeroBeat taxonomy stays in Tags, with the seeded/default tag example set to `boxing, easy, edm` instead of the prior misleading metadata-first examples. The token-lifetime slice is now implemented, QA-verified, and audit-verified: saved auth expiry is persisted/restored, obviously expired auth is pre-cleared on startup, and restore-time token rejection clears invalid saved bearer state while preserving email context for re-auth. The upload-diagnostics follow-up also diagnosed the real `create_draft_failed` seam against the configured test host and tightened the upload flow so the UI/helper no longer hide or casually trigger known-invalid draft-create requests: summary is now required locally, `metadata_blob` defaults truthfully to `{}`, logo files are preflight-checked for readable `512x288+` dimensions, failed upload attempts surface nested mod.io field errors plus `error_ref`, the testbed warning-causing preload alias was cleaned up, and auth copy now reflects the latest observed provider-returned expiry being closer to ~90 days than a guaranteed year. The binary multipart diagnostics seam is now also coder/QA/auditor verified: successful multipart and raw binary uploads keep the real `body_bytes` for dispatch, mark requests with `body_is_binary`, and expose only safe truthful previews in `body_string` instead of coercing PNG/ZIP/octet-stream bytes into UTF-8. Finally, the provider-error summarization refactor is now coder/QA/auditor verified: reusable provider error formatting lives in `AeroModIOManager`, while staged workflow context (`create_draft` / `upload_modfile` / `publish_mod`) remains in `modio_workout_upload_flow.gd` and the browser UI continues to report both pieces truthfully. The latest auth/session slice is now fully coder/QA/auditor verified as well: Test-vs-Live auth exchange/save/restore/clear all align to the active environment bucket, Test auth persists into `modio.test` instead of `modio.live`, clearing the Test bucket leaves the Live bucket intact, and the QA follow-up fix prevents cleared expiry state from being rewritten as `0`.
...
- `7676b0c` - `Fix binary multipart UTF-8 debug coercion`
