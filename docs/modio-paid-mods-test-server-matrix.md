...
| 5 | `POST /games/{game-id}/mods/{mod-id}/monetization/team` | **SKIP — out of default matrix** | Intentionally excluded from the default QA pass per plan. Harness reason: `Skipped by default; this write stays behind explicit collaborator-fixture opt-in guard`. |
| 5 | `POST /s2s/transactions/intent`, `POST /s2s/transactions/commit`, `POST /s2s/transactions/clawback` | **SKIP — out of default matrix** | Intentionally excluded from the default QA pass per plan. Harness reason: `Skipped by default; these service-token writes stay behind explicit opt-in guards`. |
