...
func test_submit_workout_runs_create_upload_publish_sequence() -> void:
	...
	assert_true(bool(recorded_requests[0].get("body_is_binary", false)))
	...
	assert_string_contains(recorded_requests[0].body_string, '[binary file omitted: %s bytes from logo.png]' % FileAccess.get_file_as_bytes(logo_path).size())
	...
	assert_true(bool(recorded_requests[1].get("body_is_binary", false)))
	...
	assert_string_contains(recorded_requests[1].body_string, '[binary file omitted: 4 bytes from workout.zip]')
