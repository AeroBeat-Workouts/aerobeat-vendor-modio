...
func test_prefers_request_raw_for_binary_multipart_bodies() -> void:
	...
	assert_true(bool(prepared.request.get("body_is_binary", false)))
	...
	assert_string_contains(prepared.request.body_string, 'name="filedata"; filename="sample.zip"')
	assert_string_contains(prepared.request.body_string, '[binary file omitted: 4 bytes from sample.zip]')
...
func test_executes_raw_multipart_part_uploads_with_query_headers_and_bytes() -> void:
	...
	assert_true(bool(_recorded_requests[0].get("body_is_binary", false)))
	assert_eq(_recorded_requests[0].body_string, "[binary body omitted: 5 bytes, content_type=application/octet-stream]")
...
func test_executes_modfile_write_requests_with_documented_multipart_form_and_delete_shapes() -> void:
	...
	assert_eq(_recorded_requests[1].headers.Authorization, "Bearer user-token")
	assert_eq(_recorded_requests[1].headers["Content-Type"], "multipart/form-data; boundary=TEST-BOUNDARY")
	assert_true(bool(_recorded_requests[1].get("body_is_binary", false)))
	...
	assert_string_contains(_recorded_requests[1].body_string, '[binary file omitted: 5 bytes from cardio-blaster-v1-1.zip]')
...
func test_executes_mod_media_requests_with_truthful_multipart_file_parts_and_form_arrays() -> void:
	...
	assert_string_contains(_recorded_requests[0].body_string, '[binary file omitted: 7 bytes from cover.png]')
	...
	assert_string_contains(_recorded_requests[0].body_string, '[binary file omitted: 7 bytes from gallery-1.jpg]')
...
func test_executes_game_media_and_collection_mod_delete_requests_with_documented_transport_shapes() -> void:
	...
	assert_string_contains(_recorded_requests[0].body_string, 'Content-Type: image/png')
	assert_string_contains(_recorded_requests[0].body_string, '[binary file omitted: 4 bytes from game-logo.png]')
	assert_string_contains(_recorded_requests[0].body_string, 'name="icon"; filename="game-icon.png"')
	assert_string_contains(_recorded_requests[0].body_string, '[binary file omitted: 4 bytes from game-icon.png]')
	assert_string_contains(_recorded_requests[0].body_string, 'name="header"; filename="game-header.png"')
	assert_string_contains(_recorded_requests[0].body_string, '[binary file omitted: 4 bytes from game-header.png]')
